...
public class OneVOneReset : BasePlugin
{
    ...
    public override string ModuleVersion => "2.6.3"; 
    ...
    public override void Load(bool hotReload)
    {
        ...
        RegisterEventHandler<EventPlayerDisconnect>((@event, info) => {
            ...
            // 只有真正在打比賽（T或CT）的人走，才會印這行中斷連線 Log
            Console.WriteLine($"[1V1 Log] 偵測到比賽中玩家 {cachedPlayerName} ({cachedSteamId}) 正在中斷連線...");

            // 在 0.0 秒立刻計算並固定當前場上打比賽的人數（T隊 + CT隊）
            int currentActiveCount = Utilities.GetPlayers().Count(p => p != null && p.IsValid && !p.IsBot && p.SteamID > 0 && (p.TeamNum == 2 || p.TeamNum == 3));

            AddTimer(1.5f, () => {
                HandlePlayerDisconnectMsg(cachedPlayerName, currentActiveCount);
    ...
    private void OnGsCommand(CCSPlayerController? player, CommandInfo info)
    {
        ...
        player.PrintToChat($" {ChatColors.Orange}可 在 聊天 欄 位 輸 入 您 要 的 武器，以 下 是 常 用 武 器");
        player.PrintToChat($" -----------------------------------------------------------------");
    ...
    private void HandlePlayerDisconnectMsg(string playerName, int activeCountAtDisconnect)
    {
        if (IsInWarmup() || _isMatchEnded) return;

        // 第一個玩家離開（原本 2 人打，走 1 個剩 1 個）：觸發廣播與中止提示
        if (activeCountAtDisconnect == 1)
        {
        ...
        // 第二個玩家離開（activeCountAtDisconnect == 0）：完全不留任何 Log 與廣播，直接無視
    }
}
